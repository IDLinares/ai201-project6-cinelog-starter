...
def test_remove_from_watchlist_removes_entry(app, sample_user, sample_film):
    """
    Removing a valid film should remove the entry from the database.
    """
    with app.app_context():
        add_to_watchlist(user_id=sample_user, film_id=sample_film)
...
def test_remove_from_watchlist_nonexistent_film_raises(app, sample_user):
    """
    Removing a film_id that doesn't exist in the database should raise
